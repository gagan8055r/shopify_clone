    ...
    const fetchAdditionalData = async (storeId) => {
        try {
            ...
            if (response.ok) {
                ...
                if (data.stores_by_pk.color_homes.length > 0) {
                    setButtonColor(data.stores_by_pk.color_homes[0].button_color);
                }
            } else {
                console.error("Error fetching additional data:", response.status);
            }
        ...
        <div className="user-component">
            ...
            {additionalData ? (
                <>
                    {activeComponent === 'home' && (
                        <HomeUserComponent
                            storeName={additionalData.stores_by_pk.store_name}
                            storeContents={getParsedContent()}
                            buttonColor={buttonColor}
                            backgroundColor={additionalData.stores_by_pk.color_homes[0].background_color}
                        />
                    )}
                    {activeComponent === 'catalog' && (
                        <div className="card-container">
